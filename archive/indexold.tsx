...
export default function Home() {
  ...
  const [currentTab, setCurrentTab] = createSignal<"all" | "solutions">("all");

  // New signal to handle execution state
  const [shouldStop, setShouldStop] = createSignal(false);

  let stepsRecord: GameStep[] = [];
  let fileInputRef: HTMLInputElement | undefined;

  // --- LOGIC: Solution Filtering & Sorting ---
  const topSolutions = createMemo(() => {
    ...
    const sorted = allResults.sort((a, b) => {
      ...
      const strictCountA = (a.path.match(/Strict/g) || []).length;
      const strictCountB = (b.path.match(/Strict/g) || []).length;
      if (strictCountA !== strictCountB) return strictCountB - strictCountA;
  ...
  const MatrixTable = (props: { step: GameStep }) => (
    <div class="space-y-4 rounded-lg border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
      <div class="flex items-center justify-between border-b pb-2">
        <h2 class="text-xs font-mono font-bold text-blue-600 dark:text-blue-400 break-all">
          {props.step.path}
        </h2>
      </div>
      ...
      <Show when={props.step.result}>
        <div
          class={`mt-2 p-2 rounded text-xs font-bold ${
            props.step.result?.includes("EQUILIBRIUM")
      ...
      <main class="w-full max-w-5xl space-y-6">
        <section class="space-y-4">
          <h1 class="text-2xl font-bold text-zinc-800 dark:text-zinc-100">
            Game Solver IEDS Program
          </h1>
          <div class="flex flex-wrap items-center gap-4">
            {/* FILE INPUT: Only visible when NO file is active */}
            <Show when={!fileName()}>
              <input
                ref={fileInputRef}
                type="file"
           ...
           dark:file:bg-zinc-800 dark:file:text-white"
              />
            </Show>
